...
import { getCopyRoleTemplate, getInsertionMode, getUserRole, getUserRoleType, getOrganization } from '../../../api/services/masterAPIs/createRoleAPI';

const RoleInformationForm = ({ values, errors, touched, handleChange, handleBlur, setFieldValue, mode = "create" }) => {
    ...
    const [hierarchyLevelOptions, setHierarchyLevelOptions] = useState([{ id: "L1", name: "L1" }, { id: "L2", name: "L2" }, { id: "L3", name: "L3" }]);
        ...
        const fetchDropdownData = async () => {
            try {
                const [
                    ...
                    organisationRes,
                    // hierarchyRes
                ] = await Promise.allSettled([getUserRoleType(), getUserRole(), getInsertionMode(), getOrganization()]);
    ...
    useEffect(() => {
        if (values?.parentRole) {
            const fetchCopyRoleTemplate = async () => {
                try {
                    const response = await getCopyRoleTemplate({ parentrole_Id: values?.parentRole }); // Make sure this API takes the parentRole ID
                ...
                {isCreateMode &&
                    <div className="form-field">
                        <CustomDropDown
                            ...
                            hasError={touched.copyRoleTemplte && Boolean(errors.copyRoleTemplte)}
